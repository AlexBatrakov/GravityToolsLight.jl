...
function get_par_file_representation(tparam::GeneralTempoParameter)
    ...
    n_value = (tparam.value isa BigFloat && tparam.value < 0) ? 33 : 32
    n_flag = 6
    n_uncertainty = 32
    ...
    if tparam.value isa BigFloat
        if abs(tparam.value) < 1e-3 || abs(tparam.value) > 1e6
    ...
    if tparam.uncertainty !== nothing
        if tparam.value isa BigFloat && (abs(tparam.value) < 1e-3 || abs(tparam.value) > 1e6)
