...
function Base.show(io::IO, gisets::GlobalIterationsSettings)
    ...
    for iter in 1:gisets.iters
        ...
        println(io, "       Fit EFACs and EQUADs: ", gisets.fit_EFACs_EQUADs[iter])
        println(io, "       Flags: ", gisets.flags[iter])
    end
	return nothing
end

function GlobalIterationsSettings(;
    keys = GlobalIterationsKeys(),
    iters,
    nits = 3 * ones(Int64, iters),
    gain = ones(Float64, iters),
    tparams_local = [Vector{GeneralTempoParameter}() for _ in 1:iters],
    fit_EFACs_EQUADs = zeros(Bool, iters),
    flags = fill("", iters),
    print_output = false
    ) 
    return GlobalIterationsSettings(
        keys,
        iters,
        nits,
        gain,
