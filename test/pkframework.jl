...
sets = Settings("/software/tempo2/T2runtime/data_ddstg")
...
GravityToolsLight.get_chisqr(obs_params, pf)
...
fig, ax = subplots()
pclm = ax.pcolormesh(pkf.grid.y, pkf.grid.x, pkf.grid.value[:chisqr] .- pkf.grid.params[:chisqr_min], cmap="Blues_r", norm = matplotlib.colors.Normalize(vmin=0.0,vmax=lvl_3sigma), rasterized=true)
