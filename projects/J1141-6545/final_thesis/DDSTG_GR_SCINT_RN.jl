...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/final_thesis/DDSTG_GR/SCINT_RN",
    version = Tempo2(),
    par_file_init = "DDSTG_GR_SCINT_RN$TNRedC.par",
...
global_iters_settings = gisets = GlobalIterationsSettings(
    ...
    nits = [5,3,3,3],
    gain = ones(N_iters),
    tparams_local = [[TP("FINISH", time_validation, flag=1), TP("TNRedFlow", TNRedFlow)] for i in 1:N_iters], 
    flags = ["" for i in 1:N_iters],
    fit_EFACs_EQUADs = [true for i in 1:N_iters],
    print_output = true
)
...
test_params = TestParameters(
    Var(name = "TNRedAmp", min = -11.5, max = -9.5, N = 9, range_rule=:lin),
    Var(name = "TNRedGam", min =   1.0, max =  5.0, N = 9, range_rule=:lin),
    ValueVariable[],
    RangeVariable[]
)

ref_sets = RefinementSettings(
    params_to_save = (:chisqr, :rms_post_fit, :rms_tn_post_fit, :AD_objective, :chisqr_full, :F0, :F1, :F2, :PB, :T0, :A1, :OM, :ECC, :PBDOT, :XDOT, :OMDOT, :M2, :MTOT, :GAMMA, :I, :IDOT),
    desired_refinement_level = 0,
    parallel = true,
    LocalMinimaUnit(:chisqr_full, from_min=true, max=300.0),
    DiffContourUnit(:chisqr_full, diffs = [10.0], contours = [lvl_1sigma], from_min=true)
    )
...
for i in 2:3
    ...
    chisqr_full_min = tf.grid.min[:chisqr_full]
    rc("mathtext",fontset="cm")
    rc("font", family="serif", size=12)
    fig, ax = subplots()
    y_delta = tf.grid.y.values[2] - tf.grid.y.values[1]
    x_delta = tf.grid.x.values[2] - tf.grid.x.values[1]
    imsh = ax.imshow((tf.grid.vars[:chisqr_full] .- tf.grid.min[:chisqr_full]), extent=(tf.grid.y.min - 0.5*y_delta, tf.grid.y.max + 0.5*y_delta, tf.grid.x.min - 0.5*x_delta, tf.grid.x.max + 0.5*x_delta), cmap="Blues_r", norm = matplotlib.colors.Normalize(vmin=0.0,vmax=1000.0), origin="lower", aspect="auto")
    cbar = colorbar(imsh)
    cs = ax.contour(tf.grid.y.values, tf.grid.x.values, tf.grid.vars[:chisqr_full] .- tf.grid.min[:chisqr_full], levels=[lvl_1sigma, lvl_2sigma, lvl_3sigma], linestyles=["-", "--", "-."], colors="red")
    plot([], [], label=L"\Delta\chi^{2} (1\sigma, 2\sigma, 3\sigma)", "-",  color="red")
    cs2 = ax.contour(tf.grid.y.values, tf.grid.x.values, tf.grid.vars[:AD_objective], levels=[0.5, 0.75, 1.0, 1.25], linestyles=["-", "--", "-.", ":"], colors="green")
    ax.clabel(cs2, inline=true, fontsize=12, fmt="%.2f")
    plot([], [], label="AD statistics", "-",  color="green")
    annot_text = L"TNRedC = $%$TNRedC,\; \chi^2_\mathrm{min} = %$chisqr_full_min$"
    ...
    legend(fontsize=12)
    tight_layout()
    savefig("saves/DDSTG_GR_SCINT_RN$(TNRedC)_200d_$i.pdf", format="pdf")

    save("saves/tf_DDSTG_GR_SCINT_RN$(TNRedC)_200d_$i.jld", "tf", tf)
