...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/full_data_thesis/DDSTG_GR/PREC_ABE",
    version = Tempo2(),
    par_file_init = "DDSTG_GR_PREC_ABE.par",
    tim_file = "J1141-6545_pn_clean.tim",
    flags = "-newpar",
    tparams = [TP("GAIN", 1.0), TP("P_DELTA", flag=1)],
    keys = BasicTempoKeys(silent=true, print_output=true, save_internal_iterations=true, fit_EFACs_EQUADs=false)
)
...
global_iters_settings = gisets = GlobalIterationsSettings(
    ...
    gain = [1.0, 0.2, 1.0],
    tparams_local = [
        [TP("P_DELTA", 0.0, flag=-1), TP("P_PHI", 0.0, flag=-1)],
        [TP("P_DELTA", 10.0, flag=1), TP("P_PHI", 45.0, flag=1)],
        [TP("P_DELTA", flag=1),  TP("P_PHI", flag=1)]
        ],
    # flags = [i < N_iters ? "" : "-nofit" for i in 1:N_iters],
    # fit_EFACs_EQUADs = [false, false, false],
    print_output = true
)
...
test_params = TestParameters(
    Var(name = "C_CHI",   min = 0.0, max = 5.0,   N = 7, range_rule=:lin),
    Var(name = "C_DELTA", min = 0.0, max = 180.0, N = 7, range_rule=:lin),
    ValueVariable[],
    RangeVariable[]
)
...
ref_sets = RefinementSettings(
    params_to_save = (:chisqr, :rms_post_fit, :rms_tn_post_fit, :F0, :F1, :F2, :DM, :PB, :T0, :A1, :OM, :ECC, :PBDOT, :XDOT, :X2DOT, :OMDOT, :OM2DOT, :M2, :MTOT, :GAMMA, :I, :IDOT, :I2DOT, :J_I, :J_DELTA, :PREC_OM_SEC, :PREC_OMDOT_SEC, :PREC_PHI, :PREC_PHIDOT, :P_LAMBDA, :P_ETA, :P_DELTA, :P_PHI, :P_PHIDOT, :ABE_X, :ABE_XDOT, :ABE_X2DOT, :ABE_PBDOT, :ABE_E, :ABE_EDOT),
