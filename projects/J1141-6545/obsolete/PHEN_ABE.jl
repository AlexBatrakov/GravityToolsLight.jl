...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/full_data_thesis/DDSTG_GR/ABE_I2DOT_OM2DOT",
    version = Tempo2(),
    par_file_init = "DDSTG_GR_ABE_I2DOT_OM2DOT.par",
...
test_params = TestParameters(
    Var(name = "P_ETA",    min = 0.0, max = 360.0, N = 7, range_rule=:lin),
    Var(name = "P_LAMBDA", min = 0.0, max = 180.0, N = 7, range_rule=:lin),
    ValueVariable[],
    RangeVariable[]
)
...
ref_sets = RefinementSettings(
    ...
    DiffContourUnit(:chisqr, diffs = [5.0], contours = [lvl_2sigma], from_min=true)
    )
...
for i in 0:3
    ...
    legend(fontsize=12)
    tight_layout()
    savefig("saves/DDSTG_GR_ABE_I2DOT_OM2DOT_$i.pdf", format="pdf")

    save("saves/DDSTG_GR_ABE_I2DOT_OM2DOT_$i.jld", "tf", tf)
