...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/final_thesis/DDSTG_GR/SCINT_DM_RN",
    version = Tempo2(),
    par_file_init = "DDSTG_GR_SCINT_DM_RN$TNRedC.par",
...
ref_sets = RefinementSettings(
    ...
    parallel = true,
    # FullUnit(:chisqr)
    LocalMinimaUnit(:chisqr, from_min=true, max=300.0),
...
for i in 0:3
    ...
    legend(fontsize=12)
    tight_layout()
    savefig("saves/GR_SCINT_DM_RN$(TNRedC)_200d_$i.pdf", format="pdf")

    save("saves/tf_GR_SCINT_DM_RN$(TNRedC)_200d_$i.jld", "tf", tf)
