...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/full_data_thesis/DDSTG_PHEN/PREC",
    version = Tempo2(),
    par_file_init = "DDSTG_PHEN_PREC.par",
...
test_params = TestParameters(
    Var(name = "J_DELTA",     min = 0.0,  max = 0.8,  N = 7, range_rule=:lin),
    Var(name = "PREC_PHIDOT", min = -8.0, max = 47.0, N = 15, range_rule=:lin),
    ValueVariable[],
    RangeVariable[]
)

ref_sets = RefinementSettings(
    params_to_save = (:chisqr, :rms_post_fit, :rms_tn_post_fit, :F0, :F1, :F2, :DM, :PB, :T0, :A1, :OM, :ECC, :PBDOT, :XDOT, :X2DOT, :OMDOT, :OM2DOT, :M2, :GAMMA, :I, :IDOT, :I2DOT, :J_I, :J_DELTA, :PREC_OM_SEC, :PREC_OMDOT_SEC, :PREC_PHI, :PREC_PHIDOT),
...
for i in 0:6
    ...
    legend(fontsize=12)
    tight_layout()
    savefig("saves/DDSTG_PHEN_PREC_$i.pdf", format="pdf")

    catch e

    end
    save("saves/DDSTG_PHEN_PREC_$i.jld", "tf", tf)
    
end

# tf = load("saves/DDSTG_PHEN_PREC_$i.jld", "tf")