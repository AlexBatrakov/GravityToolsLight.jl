...
basic_settings = settings = bsets = BasicTempoSettings(
    work_dir = "/Users/abatrakov/Documents/Work/PhD/projects/J1141-6545/full_data_thesis/DDSTG_GR_PL_RN",
    version = Tempo2(),
    par_file_init = "DDSTG_GR_PL_RN$TNRedC.par",
...
global_iters_settings = gisets = GlobalIterationsSettings(
    ...
    fit_EFACs_EQUADs = [i in (3, 5, 7, 9, 11, 12) ? true : false for i in 1:N_iters],
    print_output = true
)
...
test_params = TestParameters(
    Var(name = "TNRedAmp", min = -12.0, max = -10.0, N = 5, range_rule=:lin),
...
ref_sets = RefinementSettings(
    params_to_save = (:chisqr, :rms_post_fit, :rms_tn_post_fit, :EFACs_EQUADs_max_objective, :PL_T0, :PL_PB, :PL_ECC, :PL_X, :PL_OM, :F0, :F1, :F2, :DM, :PB, :T0, :A1, :OM, :ECC, :PBDOT, :XDOT, :X2DOT, :OMDOT, :M2, :MTOT, :GAMMA, :I, :IDOT),
...
for i in 0:3
    ...
    imsh = ax.imshow((tf.grid.vars[:chisqr] .- tf.grid.min[:chisqr]), extent=(tf.grid.y.min - 0.5*y_delta, tf.grid.y.max + 0.5*y_delta, tf.grid.x.min - 0.5*x_delta, tf.grid.x.max + 0.5*x_delta), cmap="Blues_r", norm = matplotlib.colors.Normalize(vmin=0.0,vmax=60), origin="lower", aspect="auto")
    cbar = colorbar(imsh)
    ...
    cs2 = ax.contour(tf.grid.y.values, tf.grid.x.values, tf.grid.vars[:EFACs_EQUADs_max_objective], levels=[0.43, 0.77, 1.26, 1.93], linestyles=["-", "--", "-.", ":"], colors="green")
    ax.clabel(cs2, inline=true, fontsize=12, fmt="%.2f")
    cs3 = ax.contour(tf.grid.y.values, tf.grid.x.values, tf.grid.vars[:rms_tn_post_fit], levels=[90.0, 90.5, 91.0, 91.5], linestyles=["-", "--", "-.", ":"], colors="black")
    ax.clabel(cs3, inline=true, fontsize=12, fmt="%.2f")
    # plot([], [], label=L"\Delta\chi^{2} (1\sigma, 2\sigma, 3\sigma)", "-",  color="black")
    ...
    legend(fontsize=12)
    tight_layout()
    savefig("saves/GR_PL_RN$(TNRedC)_200d_$i.pdf", format="pdf")

    save("saves/tf_GR_PL_RN$(TNRedC)_200d_$i.jld", "tf", tf)
...
# tf = load("saves/tf_GR_PL_RN84_200d_3.jld", "tf")
